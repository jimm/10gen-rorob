  ...
  module ConnectionAdapters
    ...
    class MongoPseudoConnection
      ...
      def method_missing(sym, *args)
        $stderr.puts "HERE INSIDE connection_adapters.rb MongoPseudoConnection" # DEBUG
        $stderr.puts "#{sym}(#{args.inspect}) sent to conn" # DEBUG
        $stderr.puts "start of caller list" # DEBUG
        caller(0).each { |s| $stderr.puts s } # DEBUG
        $stderr.puts "done with caller list" # DEBUG
      end
